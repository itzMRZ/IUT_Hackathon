...
const btn =
  'rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-1 active:scale-[0.98]'
...
function ToggleRow({ device }: { device: Device }) {
  ...
  return (
    <button
      ...
      aria-pressed={on}
      className={`${btn} flex w-full items-center justify-between px-3 py-2.5 text-left ${
        on
    ...
    >
      <span className="flex items-center gap-2 text-xs font-medium text-slate-700">
        <Icon size={14} className={on ? 'text-emerald-600' : 'text-slate-400'} aria-hidden />
        {device.label}
      </span>
      <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold ${on ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-600'}`}>
        {on ? 'ON' : 'OFF'}
      </span>
    </button>
  )
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-[var(--color-border)] bg-white shadow-sm">
      <div className="shrink-0 border-b border-slate-100 px-3 py-2">
        <div className="flex items-center justify-between">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[9px] font-semibold ${connected ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
      ...
      <div className="shrink-0 space-y-2 border-b border-slate-100 px-3 py-2">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {PRESETS.map((p) => (
            <button
              ...
              className={`${btn} border-slate-200 bg-white px-2 py-2 text-[10px] font-semibold text-slate-700 hover:border-violet-300 hover:bg-violet-50`}
            >
              {p.label}
            </button>
          ))}
        ...
        <button
          ...
          className={`${btn} flex w-full items-center justify-center gap-2 px-3 py-2.5 text-xs font-semibold ${
            autoSim ? 'border-sky-300 bg-sky-50 text-sky-800' : 'border-slate-200 bg-slate-50 text-slate-600'
          }`}
        >
      ...
      <div className="panel-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-2">
